...
REPORT_PATH = "./Reports"
NAME_OF_FILE = 'Emotions_MO_v4_40exp.xlsx'
...
def read_data():
    ...
    """
    final_path = os.path.join(data_path, 'yeast_clean.csv')
    data = pd.read_csv(final_path)
    ...
    X = pd.DataFrame(scaled_features, index= X.index, columns= X.columns)
    """
    final_path = os.path.join(data_path, 'emotions_clean.csv')
    data = pd.read_csv(final_path)
...
def crossover(pop,X, Y):
    ...
    while len(new_pop)!= len(pop):
        ...
        k = random.randrange(1,DIMENSION)
        rand_nums = random.sample(range(0, NUM_SOLUTIONS), 2)
        i = rand_nums[0]
        j = rand_nums[1]
        
        new_s1 = Solution((str(name) + "c"))
        new_s2 = Solution((str(name+1) + "c"))
        new_s1.pos = pop[i].pos[:k] + pop[j].pos[k:]
        new_s2.pos = pop[j].pos[:k] + pop[i].pos[k:]
        new_pop.append(new_s1)
        new_pop.append(new_s2)
        name = name+2

    sol_list = pop + new_pop

    for i in range(len(sol_list)):
        sol_list[i].evaluate(X, Y)
    
    Rank(sol_list)
    #print("list concatinated length", len(stars_list))
    sorted_stars_list = sorted(sol_list, key = lambda x: x.rank, reverse= True) 
    return sorted_stars_list[:NUM_SOLUTIONS]
...
def MultiObjective_Optimization(X:pd.DataFrame, Y:pd.DataFrame) -> None :
    ...
    while it< NUM_ITERATIONS:
        ...
        if it%5 == 0 and it > 0 :
            ...
            new_pop = crossover(new_pop, X, Y)
            #print("length of new solution = ", len(new_sol))
            #new_sol = []
...
def plotting(pop, name = None, unique = True):
    ...
    if unique == True:
        ...
        for i in range(len(pop)):
            attr.append(pop[i].num_attr)
            loss.append(pop[i].loss)
...
def evaluate_model(pop, X_train, Y_train, X_test, Y_test):
    ...
    for i in range(len(pop)):
        ...
        S.loss = loss
        print("type of loss = ",type(S.loss))
...
def single_run(experiment_id: int) -> dict[str, int]:
    ...
    plotting(rank_1_solutions, "Train.jpg", True)
    print("Plotting solutions")
    plotting(test_pop, "Test.jpg", True)
    """
    ...
    for i in range(len(test_pop)):
        loss_list.append(test_pop[i].loss)
        attr_list.append(test_pop[i].num_attr)


    metric = defaultdict()
 
    
    metric['loss_list'] = loss_list
    metric['attr_list'] = attr_list

    

    #metric['test_pop'] = test_pop
    #metric['train_pop'] = rank_1_solutions
    #print("checking loss type = ", type(test_pop[1].loss))
...
def run_experiments(num_experiments: int) -> None:
    ...
    with Pool(processes=min(num_experiments, 8, cpu_count())) as pool:
        metric = pool.map(single_run, experiment_list)
        create_report(metric)   
